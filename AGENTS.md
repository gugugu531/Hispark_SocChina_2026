...
```text
主路径:
OS08A20 -> VI -> ISP(WDR/3DNR/DRC/Gamma/CLUT) -> VPSS -> VO/HDMI
                                             -> VENC/RTSP
...
板端运行根目录：`/root/socchina-2026/`。访问方式分两种：

- 受管网络：`eth1` 获取动态 DHCPv4/DHCPv6，电脑通过全局 IPv6 连接；推荐使用
  `~/.ssh/config` 别名 `hispark-remote`。
- 电脑网线直连：`root@192.168.1.168`，作为受管网络不可用时的回退路径。

地址稳定性、Clash TUN 注意事项和恢复命令见 `docs/network-access.md`。不要把一次 DHCP
租约地址当作永久板卡地址写进脚本。Portal 认证和 networkd/systemd 安装实现不属于本仓库；
不要复制、链接或记录其外部存放位置。
...
本仓库可能公开或交付给仓库外人员。新增或修改任何源码、脚本、配置、文档、测试数据和日志前，
都必须按“公开后不会暴露人员、机构、网络或本机环境”的标准检查。

禁止写入或提交：

- 机构、学校、实验室、部门、内部项目等可识别身份信息，包括名称、缩写、SSID、域名和内部服务名。
- 账号、密码、令牌、Cookie、私钥、认证响应、用户索引、设备注册码及其编码或哈希形式。
- 可识别真实网络环境的信息，包括公网/受管网络 IP、网关、DNS、网段、交换机位置、Portal
  地址、MAC 查询参数和一次性 DHCP 租约。
- 个人用户名、主目录、工作区绝对路径、主机名，以及只在某位开发者机器上成立的路径。
- 原始日志、命令输出、截图和抓包中附带的上述信息。引用结果前先提炼并脱敏。

仓库内统一使用中性占位符和泛化名称，例如：
...
- 仓库 Markdown 链接只能指向仓库内文件；不要链接到父目录、个人目录、私有站点或仓库外工作区。
- 认证实现、凭据、机器专用配置和其他私有运维资产必须留在仓库外；仓库不得记录其具体存放位置。
- 源码和脚本不得写死仓库外路径。外部 SDK、数据集和工具通过环境变量或必填命令行参数传入。
- 历史实验若包含敏感原始值，只记录泛化后的测量口径与结论；不要为了“可复现”保留真实身份或网络标识。

命名与板端清理：

- 新服务、目录、脚本和 SSH 别名使用中性功能命名，不包含机构、地点或人员信息。
- 若板端遗留名称会暴露身份，可在确认无运行依赖、完成新路径验证后重命名或清理。
- 迁移凭据时不得打印、读取到助手输出或复制进仓库；文件权限应限制为仅必要用户可读。

提交前至少执行一次针对敏感关键词、个人绝对路径、具体受管网络地址和仓库外链接的扫描。
发现命中时先判断是否确属必要公开信息；默认做法是删除、泛化或改为运行时参数，而不是加入例外。
...
- 不要提交或链接机构身份、真实受管网络参数、认证信息、个人路径或其他仓库外私有资产。
