...
**R v2.1 容量路线证伪（2026-07-05）**：48ch/6blk（参数 ×2.5）+ 240ep 重训，
@160ep val=21.98 vs v2.0 的 21.95——**+0.03 dB**，且训练在 ep160 后 CPU 忙等卡死
（GPU 0%，9.5h 无输出，已终止；ckpt 保留 ep160 最优）。结论：~22 dB 是当前
（1920 对样本 × 宽域）的收敛区，瓶颈不是容量——宽域真实纹理的细节残差
（demosaic/锐化/局部行为）对全局 θ 条件网络存在不可约成分。
**决策：代理保真度路线到此为止，转蒸馏主路径**（θ\* 硬件标签绕开代理保真依赖；
§5.9 已证 θ\* 在过曝方向优于当前网络 3.5–4 dB；`hw_search.py` 就绪）。
过程坑记录：48ch/6blk 全帧 batch16 在 8GB 显存 OOM（→batch 8）。
