...
| 级别 | 路线 | 一句话 | 实时性 | 方向 | 状态 |
| --- | --- | --- | --- | --- | --- |
| **唯一产品路线** | CoTF-inspired 参数网 + ISP 参数块 | NN 只出 RGB CLUT；AE 规则驱动 Gamma；DRC/LDCI 由 ISP 管理 | ✅ 1024x576 可 | 双向 | 生产闭环与失败降级已完成；画质待验收 |
| **已关闭** | 曲线网络 ExpoCurveNet（整图增强） | 单段全分辨率 NN 直接出增强帧 | 🟡 仅 768x432 | 双向 | 不实现产品路径，只保留实验数据 |
| **兜底** | Zero-DCE（Lite） | 探索层已板端实测的单向低光增强 | ✅ 640x320≈30ms | **单向（仅提亮）** | 已有 OM 与实测基线，需场景门控 |

产品不再实现 Config-Q、ExpoCurveNet 整图运行、`postproc/compose` 或严格同帧分屏。若双向方案最终
画质不达标，只允许退到已有 Zero-DCE Lite 兜底，不重新打开整图双向路线。
...
- ExpoCurveNet `768x432 + shared niter8` 单次约 `27.2ms`；该数字仅作为关闭整图路线的决策证据。
...
- 修正后的 17v2 RGB bridge 已在 `test_paramnet_live` 动态预览：每 30 帧推理并热刷完整 CLUT，
  20 秒 604 帧、20/20 更新成功；高光/端点/单调/步长护栏和生产高光门控已接入，仍待多场景画质验收。
...
ExpoCurveNet、Config-Q 和整图增强只保留历史模型与测速记录，不再进入产品数据通路。
...
若当前双向 CLUT 方案在画质或工期上不达预期，退回探索层已在板上跑通的 **Zero-DCE Lite FP16 OM**
（同芯片族 SVP_NNN/ACL 实测基线）：

- 已知实测（离线单算 OM 执行，口径见根工作区 `AGENTS.md`）：`640x320 ≈ 29.9ms`、`1024x640 ≈ 96ms`、
  `160x160 ≈ 3.8ms`；本仓库横评复测 `640x360 ≈ 33.9ms`、`320x180 ≈ 8.5ms`（与历史一致，交叉验证测速工具）。
  单算 OM 全分辨率表见根 `docs/zero-dce-lite-svp-nnn-resource-usage.md`。注：另有 `160x160 + AIPP ≈ 1ms`
  的低分辨率量级，是 AIPP 融合后的口径，**不可与上述单算 OM 数字直接对比**。
...
6. 板端测试自动化、交互模块和交付收尾；余量 NPU 感知→控制属于后续增强项。
