...
5. 🟡 阈值标定：`control_decide` 已重构为**高光优先** + **AE 统计域重标定**。
   - 板端实测定位过曝根因：AE 自动曝光把 `mean_luma` 钉在设定点 ≈53（日志 `luma=52.5`），
     而旧 `DARK_LUMA_TH=60` 高于设定点 → 每个 AE 稳定场景都被误判"暗"→ 永远提亮。
   - 重标定以设定点 53 为基线（DARK 60→40、新增 BRIGHTEN 天花板 62、BRIGHT 95），裁剪比例为主判据。
   - **板端前后验证**（同场景 AE luma≈53）：判决 `BRIGHTEN → BYPASS`，输出 Y 均值 `146.9 → 115.3`、
     高光裁剪 `2.01% → 1.86%`，过度提亮消除。证据见 `artifacts/quality-validation-2026-06-24/`。
   - **单锚点限制**：DARK/BRIGHT 精确值仍需补"真实暗场 / 真实过曝场景"的 AE↔输出配对确认。
