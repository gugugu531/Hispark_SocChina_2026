...
5. 🟡 阈值标定：`control_decide` 已重构为**高光优先**（过曝不可逆，新增提亮抑制阈值与亮度天花板，
   COMPRESS 高光阈值 3.0%→1.5%，主机单测覆盖；旁路态 Y≈109/clip≈2.5% 不再误判提亮）。
   余：用输出域（RTSP 抓帧）实测的 mean/clip 对各数值阈值做最终标定（AE 统计域与输出亮度域未必对齐）。
