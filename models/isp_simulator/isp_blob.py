...
Blob 格式 (little-endian):
    ...
    -- LDCI (if flags & 2) --
    486-489: ldci_enable (uint32)
    490-492: ldci_he_pos_wgt[3] (uint8 × 3)
    493-495: ldci_he_neg_wgt[3] (uint8 × 3)
    496-497: ldci_blc_ctrl (uint16)
    498: ldci_gauss_lpf_sigma (uint8)
"""
...
def sim_params_to_blob(params: torch.Tensor, enable_drc: bool = True,
                       enable_ldci: bool = True) -> bytes:
    """将模拟器 96 维参数向量编码为 ISP blob 二进制。

    Args:
        params: (1, 96) 或 (96,) 模拟器参数，值域 [0, 1]
        enable_drc: 是否启用 DRC
        enable_ldci: 是否启用 LDCI
    ...
    # Header
    buf += struct.pack('<III', 0x49535000, 1, flags)

    # DRC
    if enable_drc:
        buf += struct.pack('<I', 1)  # drc_enable
    ...
    # LDCI
    if enable_ldci:
        buf += struct.pack('<I', 1)  # ldci_enable
        ld = p["ldci"]
...
def test_roundtrip():
    ...
    assert len(blob) == 499, f"Expected 499 bytes, got {len(blob)}"

    # 非恒等参数
    params2 = make_identity_params(1)
    params2[0, 65:71] = torch.tensor([0.0, 0.2, 0.5, 0.5, 0.8, 1.0])  # S-curve
    blob2 = sim_params_to_blob(params2)
    assert len(blob2) == 499
    print("Roundtrip OK")
