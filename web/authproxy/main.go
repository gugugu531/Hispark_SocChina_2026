...
//   - 反向代理到后端 web 控制台
//
// 用法：socchina-auth [-addr :8080] [-backend http://127.0.0.1:8090] [-pass socchina2026]
package main
...
func main() {
	...
	pass := flag.String("pass", "socchina2026", "login password")
	flag.Parse()
	...
	http.HandleFunc("/api/v1/status", func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		w.Header().Set("Access-Control-Allow-Origin", "*")
		conn, err := net.DialTimeout("unix", "/tmp/socchina-app-control.sock", 2*time.Second)
		if err != nil {
...
const BOARD='[2001:250:4000:822c:c634:4d5d:56b1:a1da]';
const PRESETS={
...
async function tryWebRTC(){
  ...
  try{
    ...
    pc.onicecandidate=e=>{
      if(e.candidate)return;
      fetch("http://[2001:250:4000:822c:c634:4d5d:56b1:a1da]:8889/live/whep",{
