...
files=(
    ...
    "${REPO_ROOT}/scripts/board/socchina-health"
    "${MODEL_FILE}"
)
...
ssh "${BOARD}" "set -eu
    ...
    install -m 0755 '${STAGE}/socchina-health' /usr/local/sbin/socchina-health
    ...
    install -m 0644 '${STAGE}/runtime.conf' /usr/share/socchina/runtime.conf.example
    ...
    for _attempt in 1 2 3 4 5; do
        if /usr/local/sbin/socchina-health; then
            health_ok=1
            break
        fi
        sleep 2
...
echo "[install-service] 健康检查: ssh ${BOARD} socchina-health"
